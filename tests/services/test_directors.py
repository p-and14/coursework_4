...
class TestDirectorsService:
    ...
    def director(self, db):
        obj = Director(name="Тейлор Шеридан")
        db.session.add(obj)
        db.session.commit()
        return obj

    def test_get_director(self, directors_dao_mock, directors_service, director):
        assert directors_service.get_item(director.id) == directors_dao_mock.get_by_id.return_value
